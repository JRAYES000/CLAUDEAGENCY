...
**Pourquoi :** demande de Julien. Un live d'Emilio Business du 14/05/2026 montre Claude construisant un tunnel de vente entier dans Tiny Pages, autour d'une offre parodique (« l'Académie des Branleurs Nomades », formation à cumuler plusieurs emplois remote en laissant des agents IA travailler). Julien a demandé d'en reprendre **uniquement la page de vente**, son architecture et son registre direct response, en substituant à l'offre parodique une offre réelle de Claude Agency : une heure d'audit en visio, puis mise en place de l'automatisation par l'équipe.
**Fait :** page Astro `app/src/pages/deleguer-son-bullshit-job.astro`, bâtie sur `BaseLayout` (elle hérite donc du header, du pied de page, des polices, du GA4 et du Schema d'organisation). Dix sections reprenant l'ordre de la page source : accroche, constat sur le quotidien, bandeau terracotta plein, mécanisme, méthode en trois étapes, ce que vous obtenez, familles de tâches automatisées, pour vous / pas pour vous, FAQ en six objections, appel final. Charte `DESIGN.md` appliquée aux tokens (`cream-50`/`cream-100`, `sand`, `ink`, `muted`, rampe `brand-*`), aucun em dash dans la copy, `text-balance` sur les titres, `focus-visible:ring` sur les interactifs. Les deux appels pointent vers `/contact/` et `/diagnostic/`. Quatre illustrations générées par Cloudflare Workers AI (FLUX.2 klein 9B, coût 0 $, journalisées dans `~/.claude/generations.jsonl`), dans la palette terracotta et crème du site, registre humoristique : salarié enseveli sous les dossiers, tableur qui se déroule dans le couloir, petit robot au clavier devant une chaise vide, personne qui récupère son après-midi. Lien ajouté dans `Footer.astro`, section « Services », en neuvième position sous « Outils IA sur mesure ».

**Trois écarts assumés par rapport à la page source**, tous décidés au moment de la mise en ligne :
1. **Les trois témoignages ont été supprimés**, pas réécrits. La page source les fait générer par l'IA avec prénoms, âges, villes et montants. Publier de faux avis sur un site marchand tombe sous l'article L121-2 du code de la consommation. Ils sont remplacés par une section « ce qu'on automatise le plus souvent », qui décrit des familles de tâches et n'attribue aucune parole à personne. **Ne pas les réintroduire sans clients réels et consentants.**
2. **Le compte à rebours et la rareté ont été supprimés.** La source affiche un minuteur de 17 jours et un nombre de places inventé. Un faux compte à rebours relève de la même qualification. Si Julien veut une rareté sur cette page, elle doit porter sur un nombre de créneaux réellement tenu, comme le compteur à la main de `/semaine-offerte/`.
3. **La page vouvoie**, alors que la source tutoie. `PRODUCT.md` fixe la deuxième personne de politesse pour toute la voix de marque ; une page qui tutoie au milieu d'un site qui vouvoie est une rupture visible. Le mordant du registre direct response est conservé, le tutoiement non.

**Mesure :** `npm run build` vert, code de sortie 0, **93 pages** contre 92 avant (entrée 113). Les quatre images converties en WebP par Astro (259 ko → 60 ko pour la plus lourde en 800 px). Page présente dans `dist/deleguer-son-bullshit-job/index.html` et dans `sitemap-0.xml` ; lien du pied de page vérifié dans le HTML rendu de l'accueil ; les quatre images servies en HTTP 200 sur le serveur de préversion, contrôlées une par une par leur URL `/_astro/`.
**Aucun mot-clé ciblé, aucun volume mesuré.** Cette page n'est pas née d'une requête de `REQUETES.csv` : elle vient d'une demande directe. Elle n'y est donc pas inscrite et aucune ligne n'a été modifiée. Si elle doit devenir un actif SEO, le travail de mesure reste entièrement à faire.
**Non vérifié :** l'indexation, la position, et l'effet du titre en SERP. Également non vérifié, et c'est le point qui mérite un arbitrage avant de laisser vivre la page : **elle s'adresse à un salarié, alors que tout le reste du site s'adresse à des employeurs** (dirigeants de PME, organismes de formation). Le pied de page la présente parmi huit prestations vendues à des entreprises. Personne n'a mesuré l'effet de cette dissonance sur un visiteur dirigeant qui atterrit dessus depuis le pied de page.
**Suite :** trois choses. (1) L'arbitrage de cible ci-dessus, à trancher par Julien : soit la page est repositionnée vers l'indépendant et le dirigeant qui veut récupérer ses propres journées, soit elle reste au salarié et on assume qu'une page du site parle à l'autre camp. (2) Le sitemap n'exclut aucune page (règle du 23/08) : celle-ci est donc indexable dès la mise en ligne, sans geste supplémentaire. (3) `.claude/launch.json` a été créé sur le poste pour la préversion locale et reste invisible de git, comme tout `.claude/` : à forcer avec `git add -f` si une session cloud doit s'en servir.

---

## 2026-09-19 (113) — L'image sociale du site disait claudepartners.fr, et l'accueil ne parlait qu'aux OF
**Type :** correctif technique + titre de l'accueil
**URLs :** `https://claudeagency.fr/` et toutes les pages servant l'image sociale par défaut ; `https://claudeagency.fr/barometre-ia-organismes-formation/`
**Pourquoi :** en refaisant les liens en vedette du profil LinkedIn de Julien, deux défauts sont apparus, tous deux invisibles depuis le site lui-même. (a) `app/public/og-default.jpg` — l'image servie en `og:image` par **toutes** les pages sans image propre — portait la mention **claudepartners.fr** en pied et le titre « Une équipe d'experts IA pour les organismes de formation » : chaque partage d'une page de claudeagency.fr sur LinkedIn, WhatsApp ou Slack affichait l'URL de l'autre produit. (b) Le `<title>` de l'accueil, « Agence IA et marketing pour organismes de formation », est le libellé que LinkedIn reprend sur le lien en vedette et que Google affiche en SERP — incompatible avec la bascule vers une cible plus large décidée par Julien le 2026-08-22 et engagée par les entrées 110 et 111.
**Fait :** nouvelle image sociale générique rendue en Chrome headless depuis une source HTML (`~/Claude/assets/og/og-default.html`, charte terracotta de `DESIGN.md`, mêmes polices que la bannière LinkedIn) — « Votre marketing et votre administratif, pris en charge », ligne de métiers SEO · SEA · Site · Prospection · Administratif, bandeau `claudeagency.fr` et « 100 € HT la journée de consultant ». Publiée en `og-default.png` (1200×630) **et** réécrite sur `og-default.jpg` au même nom, pour que les aperçus déjà en cache chez LinkedIn et WhatsApp servent le nouveau visuel sans attendre un nouveau scrape. `BaseHead.astro` : défaut basculé sur `.png` et `imageAlt` par défaut dégenré. `semaine-offerte.astro` pointait `og-default.jpg` en dur dans son propre `<head>` — corrigé. Titre de l'accueil : « Agence IA et marketing pour entreprises » (39 car., donc le suffixe de marque s'applique et le `<title>` rendu devient « Agence IA et marketing pour entreprises · Claude Agency » — 55 car., sous le seuil de troncature) ; description réécrite avec le tarif. La page Baromètre garde une image sociale **dédiée** et assumée OF (`og-barometre.png`, « Où en sont vraiment les organismes de formation avec l'IA ? »), arbitrage de Julien : le contenu est OF, son visuel doit l'être.
**Mesure :** les deux requêtes que l'accueil porte dans `REQUETES.csv` au relevé du 2026-09-07 — « claude agency » (marque, position 42,0 / 4 clics / 227 impressions) et « agence marketing claude » (position 1,9 / 0 clic / 36 impressions). Le nouveau `<title>` conserve les trois termes de la seconde (« agence », « marketing », « Claude ») et ajoute « Claude Agency » en clair, absent de l'ancien : rien n'est retiré, la marque est renforcée. Aucune position post-changement relevée — le prochain relevé du lundi tranchera. `npm run build` vert, 92 pages, code de sortie 0 ; balises `og:image` vérifiées dans `dist/` sur l'accueil, le Baromètre, `/diagnostic/`, `/semaine-offerte/` et `/evaluation-claude-code/`.
**Non vérifié :** l'effet du changement de titre sur les deux requêtes suivies. Un `<title>` se réécrit en SERP au gré de Google et la position 1,9 de « agence marketing claude » repose sur 36 impressions en 28 jours — trop peu pour que la prochaine mesure soit concluante à elle seule.
**Suite :** deux choses. (1) Les vignettes des liens en vedette LinkedIn sont **figées au moment où le lien est ajouté** : le déploiement ne les met pas à jour, il faut retirer et re-ajouter chaque lien — fait dans la foulée, dans l'ordre arbitré par Julien (Semaine offerte, Diagnostic, Accueil, Évaluation, Baromètre). (2) `app/public/og-default.jpg` et `og-default.png` portent désormais le même visuel mais le `.jpg` n'est plus référencé par aucune page : le garder est volontaire, il ne sert qu'aux aperçus déjà en cache. Ne pas le supprimer en croyant nettoyer un fichier mort.
...
**Pourquoi :** carte 219 de `visibilite-ops`. 50 des 57 articles du blog parlaient de Qualiopi, de conventions de formation et d'émargement : un dirigeant de PME qui tombait dessus comprenait en trois secondes que le site n'était pas pour lui. Les volumes de l'entrée 110 débloquaient l'écriture. Le socle organismes de formation n'a pas bougé d'un mot.
**Fait :** deux des quatre requêtes `a-ecrire` de l'entrée 110 traitées, les deux plus gros volumes — « automatiser tâches » (720) et « gestion administrative entreprise » (210). Brief de chacune sur les trois premiers résultats organiques et sur Google Suggest le 2026-09-18. Maillage posé **à la main**, jamais par script : pied de page (donc toutes les pages), corps de `content/services/automatisation.mdx`, `blog/agence-claude-comment-choisir.mdx` vers « automatiser tâches », et les deux articles se lient entre eux. Chacun sort vers `/calculateur-economies-ia/` (page de la carte 217), `/services/automatisation/` et `/diagnostic/`. Images **réemployées** du réservoir (`agence-claude-choisir-cover.jpg` et `charge-administrative-cover.webp`), aucune génération. Aucun schéma `FAQPage` ni `HowTo` ajouté. 1 084 et 1 055 mots.
**Mesure :** chiffres publics, tous ouverts et relus à la source le 2026-09-18 avant publication — Insee enquête TIC entreprises 2024 publiée le 14/10/2025 (10 % des entreprises de 10 salariés ou plus utilisent l'IA en 2024 contre 6 % en 2023 ; 9 % chez les 10-49, 15 % chez les 50-249 ; usage « organisation des processus d'administration » 11 % → 24 %) ; Baromètre France Num 2025 de la DGE, septembre 2025 (26 % des TPE-PME, doublé en un an, enquête Crédoc sur 11 021 entreprises) ; rapport d'information du Sénat n° 743 du 15/06/2023 (4 à 8 h d'administratif par semaine pour 40 % des chefs d'entreprise, plus de 8 h pour 30 % ; 1 à 3 % du chiffre d'affaires pour 54 % des TPE ; estimation OCDE de 60 Md€ soit 2,8 % du PIB 2007). Les quatre URL vérifiées en HTTP 200 sans `-L`. Aucune position relevée à ce jour sur les deux requêtes.
**Non vérifié :** le bloc « Autres questions posées » de Google n'a pu être récupéré mot pour mot par aucun outil de la session (WebSearch ne l'expose pas, la lecture directe de Google est bloquée par l'écran de consentement). Les sous-titres reprennent donc les formulations de Google Suggest du 2026-09-18, pas des questions PAA. Le contrôle 8 de la compétence `redaction-seo` est en échec assumé sur ces deux pages.
**Suite :** à savoir avant d'écrire la troisième page. « Automatiser tâches » est la seule requête à gros volume du lot, mais son intention est en grande partie technique : les suggestions Google du 2026-09-18 sortent Excel, Windows, PowerShell, Python et OFPPT, et le haut de page organique est tenu par des éditeurs de logiciels qui publient 2 200 à 5 200 mots (Bitrix24, IBM, Asana). La page ne prend pas ce terme de front : elle est posée sur la traîne qualifiée de l'intention dirigeant (coût et rentabilisation, données et RGPD, faire soi-même ou confier), trois angles absents des trois premiers résultats. Ne pas conclure à l'échec sur la position du terme nu. Restent `a-ecrire` : « ia productivité » (110) et « intelligence artificielle pme » (50) — une seule des deux fera la troisième page de la carte 219.
