...
> Liens : PDF = `https://claudeagency.fr/ressources/10-automatisations-ia.pdf` · Audit = `https://claudeagency.fr/contact/`
...
3. Définir la **thank-you page** du widget sur `https://claudeagency.fr/merci-ressource/`.
4. Copier le code du widget (**View code**) → renseigner `app/src/data/mailjet.ts` (`actionUrl`, `fields`, `hidden`).
5. Vérifier le **domaine expéditeur** `claudeagency.fr` (déjà authentifié).
