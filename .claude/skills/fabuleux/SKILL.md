...
## Intégration ClaudePartners : router les sous-agents sur Haiku

Tenir `fabuleux` ne veut pas dire tout faire sur le modèle fort. Le travail mécanique part vers un
sous-agent Haiku, le jugement reste sur le modèle fort (voir `CLAUDE.md` pour la politique complète).

- **→ Sous-agent Haiku (`seo-researcher`)** : ancrage et collecte de la boucle universelle (recherche
  dans le dépôt, `git`/`grep`, audits mécaniques de balises et de liens, collecte de données SEO,
  captures à regarder). Tâches vérifiables, sans jugement.
- **→ Modèle fort (fil principal)** : les routes qui engagent la qualité. La route PROSE
  (soustraction, anti-slop, ponctuation FR), les arbitrages, le choix du maillage, la validation
  finale, la lecture des screenshots avec la vision.

Ne pas activer `CLAUDE_CODE_SUBAGENT_MODEL=haiku` : ce flag forcerait *tous* les sous-agents (dont
la rédaction) sur Haiku et casserait la qualité. Le routage reste sélectif, par type de tâche.
...
- Le travail mécanique est-il bien parti sur Haiku, le jugement gardé sur le modèle fort ?
